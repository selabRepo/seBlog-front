...
class Header extends Component {
  ...
  render() {
    const headerList = HEADER_LINK.map((headerLi, index) => (
      <Link href={headerLi.link} key={index}>
      ...
      <>
        <nav className={this.state.scroll > this.state.top ? 'fixed-nav' : ''}>
          <ul className={'header_ul'}>
            <li style={{ width: 400 }}>
              <h2 className="h1-responsive font-weight-bold text-center my-2">SE LAB</h2>
            </li>
            {headerList}
          </ul>
        </nav>
